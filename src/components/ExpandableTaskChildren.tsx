...
export const ExpandableTaskChildren: React.FC<ExpandableTaskChildrenProps> = ({ task, users, onUpdateTask }) => (
  <div className="border-x border-b border-zinc-800 bg-[#0e0e12]">
    <div className="flex items-center gap-2 border-b border-zinc-800/70 px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-zinc-500"><ListTree size={12} />Subtarefas <span className="font-mono">{task.subtasks.filter(item => item.completed || item.status === 'CONCLUIDO').length}/{task.subtasks.length}</span></div>
