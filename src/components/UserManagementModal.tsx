...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: { name: string; email: string; password?: string; role?: UserRole; job_title: string; status: 'ACTIVE' | 'INACTIVE'; avatarFile?: File; removeAvatar?: boolean }) => Promise<void>;
  currentUser: User;
  user?: User | null;
}
...
export const UserManagementModal: React.FC<Props> = ({ isOpen, onClose, onSave, currentUser, user }) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setError(null);
    try {
      await onSave({ name: name.trim(), email: email.trim(), password: password || undefined, ...(!isSelf ? { role } : {}), job_title: jobTitle.trim(), status, avatarFile: avatarFile || undefined, removeAvatar });
      onClose();
    } catch (saveError: any) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div className="relative w-full max-w-lg bg-[#121216] border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden">
      ...
      <form onSubmit={submit} className="p-5 space-y-4 text-xs">
        <AvatarUploadField name={name || 'Novo usuário'} src={user?.avatar} selectedFile={avatarFile} removed={removeAvatar} onSelectedFile={file => { setAvatarFile(file); setRemoveAvatar(false); }} onRemove={() => { setAvatarFile(null); setRemoveAvatar(true); }} disabled={saving} compact />
